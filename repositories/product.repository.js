...
class Products {
  ...
  async getAllProducts ({ res, req }) {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: 'No se pudo obtener los productos' })
    }
  }
  ...
  async getProductById ({ res, req, id }) {
    ...
    let findProducts = []
    try {
      findProducts = await this.product.find({ pid: id })
    } catch (error) {
      console.log('Error al encontrar los productos', error)
      return res
        .status(400)
        .json({ error: 'No se pudo obtener el producto' })
    }
    ...
    if (product) {
      ...
      if (stock === 1) {
        return res.status(400).json({
          error: 'No hay mas stock disponible para este producto'
        })
      }
  ...
  async createProduct ({ res, req }) {
    ...
    try {
      ...
      return res
        .status(201)
        .json({ success: 'Se creo el producto correctamente' })
    } catch (error) {
      console.log(error)
      return res
        .status(400)
        .json({ success: 'No se pudo crear el producto' })
    }
  }
    ...
    try {
      ...
      products = products.map((p) => {
        return {
          ...
          pid,
          stock: {
            [Object.keys(p._doc.stock)[0]]:
                            stocks[Object.keys(p._doc.stock)[0]]
          }
        }
      })
      ...
      res.status(201).json({
        success: 'Se actualizo el producto correctamente'
      })
    } catch (error) {
      console.log(error)
      return res
  ...
  async deleteProduct ({ res, req }) {
    ...
    try {
      ...
      res.status(201).json({
        success: 'Se elimino el producto correctamente'
      })
    } catch (error) {
      console.log(error)
      return res
        .status(400)
        .json({ error: 'No se pudo eliminar el producto' })
    }
  }
}
